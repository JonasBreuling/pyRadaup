...
for i, sol in enumerate(sols):
    ...
    for ivar in range(Xini.size):
        errors[ivar, i] = abs(
            (sol.y[ivar, -1] - sol_ref.y[ivar, -1]) / sol_ref.y[ivar, -1]
        )
        interped_sol = sol_ref.sol(sol.t)
        rel_error = (sol.y[ivar, :] - interped_sol[ivar, :]) / (
            1e-10 + abs(interped_sol[ivar, :])
        )
        errors_st[ivar, i] = (1 / tf) * scipy.integrate.trapezoid(
            abs(rel_error) ** p, sol.t
        ) ** (1 / p)
...
for ivar in range(Xini.size):
    plt.semilogx(
        dt_vec,
        np.gradient(np.log(errors[ivar, :]), np.log(dt_vec)),
        label=f"var {ivar}",
        marker=".",
    )
    plt.xlabel("dt")
...
for ivar in range(Xini.size):
    plt.semilogx(
        dt_vec,
        np.gradient(np.log(errors_st[ivar, :]), np.log(dt_vec)),
        label=f"var {ivar}",
        marker=".",
    )
    plt.xlabel("dt")
