...
assert (
    sum((c is None for c in chosen_vals)) == 2
), "cannot use more than 2 varying parameters at once"

# find the subset of runs
Iselect = np.where(
    np.logical_and(
        *(
            data[chosen_keys[ic]] == c
            for ic, c in enumerate(chosen_vals)
            if c is not None
        )
    )
)[0]
...
for val in values:
    ...
    plt.plot(
        data[varying_keys[1]][I],
        data[reskey][I] * multiplier,
        marker=".",
        label=f"{vark}={val}",
    )
