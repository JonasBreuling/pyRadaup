...
solfort = integration.radau5(
    ...
    max_ite_newton=max_newton_ite,
    bUseExtrapolatedGuess=bUseExtrapolatedGuess,
    bUsePredictiveController=bUsePredictiveController,
    safetyFactor=safetyFactor,
    deadzone=deadzone,
    step_evo_factor_bounds=step_evo_factor_bounds,
    jacobianRecomputeFactor=jacobianRecomputeFactor,
    newton_tol=newton_tol,
    mass_matrix=mass,
    var_index=var_index,
    bPrint=bPrint,
    nMaxBadIte=max_bad_ite,
...
solpy = solve_ivp(
    ...
    mass_matrix=mass,
    bPrint=bPrint,
    max_newton_ite=max_newton_ite,
    min_factor=step_evo_factor_bounds[0],
    max_factor=step_evo_factor_bounds[1],
...
for sol, name, linestyle, marker, color in [
    ...
    plt.plot(
        sol["t"], sol["y"], color=color, linestyle=linestyle, marker=marker, label=None
    )  # r'$x_{{}}$'.format(name))
...
for sol, name, linestyle, marker, color in [
    ...
    plt.plot(
        sol["t"], sol["T"], color=color, linestyle=linestyle, marker=marker, label=None
    )  # r'$x_{{}}$'.format(name))
    plt.plot(
        np.nan, np.nan, linestyle=linestyle, color=color, marker=marker, label=name
    )
